...
sub getStats {
  ...
  foreach my $fileName (`ls ${path}/*${algorithm}*.allHV`) {
    chomp($fileName);
...
sub getRankings {
  my ($algorithm, $numberRuns, $path) = @_;
  my @fileNames = `ls ${path}/*${algorithm}*.allHV`;
  my $rankings = {};
...
print("${path}\n");
my $statistics = getStats $algorithm, $path;
my $rankings = getRankings $algorithm, $numberRuns, $path;
